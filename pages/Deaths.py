...
def load_data() -> pd.DataFrame:
    DATA_DIR = r"data\time_series_covid_19_deaths.csv"
...
def transform_data_for_scatter(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_sct = df_sct.melt(id_vars=['Country/Region', 'Lat', 'Long'], value_name='Total Deaths', var_name='Date')
    return df_sct

df = load_data()
df_sct = transform_data_for_scatter(df=df)

st.markdown("# Analysis of Deaths")
st.write(
    """
    On this page, we provide visualizations like maps or barplot with respect to the total deaths
...
@st.fragment
def barplot():
    st.subheader("Total Deaths by Countries")
    st.bar_chart(
        data=df_sct.loc[df_sct["Country/Region"].isin(country_select), ["Total Deaths", "Country/Region"]]\
