...
def load_data() -> pd.DataFrame:
    DATA_DIR = r"data\time_series_covid_19_confirmed.csv"
...
def transform_data_for_scatter(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df_sct = df_sct.melt(id_vars=['Country/Region', 'Lat', 'Long'], value_name='Total Cases', var_name='Date')
    return df_sct

df = load_data()
df_sct = transform_data_for_scatter(df=df)

st.markdown("# Analysis of Cases")
st.write(
    """
    On this page, we provide visualizations like maps or barplot with respect to the total cases
...
st.subheader("Total Cases by Countries")

@st.fragment
def barplot():
    st.bar_chart(
        data=df_sct.loc[df_sct["Country/Region"].isin(country_select), ["Total Cases", "Country/Region"]]\
